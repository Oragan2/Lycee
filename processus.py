...
def graph(ls:list,md:str):
    ...
    #### Obtient l'enchainement des executions suivant le mode demander
    for d in ls:
        tt += d.get_duree()
    if md == "RR":
        r = RR(ls)
    ...
    for d in ls:
        ...
        for i in range(tt):
            if r[i] == d.get_name(): # Pour afficher les lignes pour le graphique
            ...
            if i == d.get_arrive():
                P1.draw(canvas,'red') # Affiche le point rouge de quand le processus est initialisée
        ...
        P1 = Point(25,P1.y-ds) # Fais monter d'une ligne les points
        P2 = Point(25+dls,P2.y-ds)
